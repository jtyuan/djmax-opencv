...
def speed_direction_batch(dets, tracks):
    ...
    dx = np.divide(dx, norm, out=np.zeros_like(dx), where=norm != 0)
    dy = np.divide(dy, norm, out=np.zeros_like(dy), where=norm != 0)
...
def associate(
    ...
    iou_matrix: np.ndarray = iou_batch(detections, trackers)
